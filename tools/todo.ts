    ...
    .array(
      z.object({
        ...
        estimatedEffort: z
          .string()
          .optional()
          .describe("Estimated time/effort (e.g., '30 min', '2 hours')"),
      }),
    )
    ...
    .array(
      z.object({
        ...
        estimatedEffort: z
          .string()
          .optional()
          .describe("Estimated time/effort"),
        completed: z.coerce
          .boolean()
          .describe("Whether this item is completed"),
      }),
    )
...
async function getTodoWithItems(
  todoId: string,
  userId: number,
...
export const todoTools = {
  ...
  todo_update: {
    description:
      "Update an existing todo list by replacing all its items. Use this when the user wants to add new tasks, remove tasks, reorder tasks, or modify task details. You need the todo ID (from todo_list or todo_create) and the complete new list of items.",
    ...
    handler: async (args: z.infer<typeof UpdateTodoSchema>, userId: number) => {
      ...
      const asyncDb = getAsyncDatabase();
      if (asyncDb) {
        await asyncDb.run("DELETE FROM todo_items WHERE todo_id = $1", [
          args.todoId,
        ]);
        ...
        db.run("UPDATE todos SET updated_at = ? WHERE id = ?", [
          Math.floor(Date.now() / 1000),
          args.todoId,
        ]);
      }
  ...
  todo_list: {
    description:
      "Show all todo lists the user has created. Use this when the user asks to see their todos, check what tasks they have, or when you need to find a todo ID for other operations. Returns list titles, progress, and IDs.",
    schema: z.object({}),
    handler: async (_args: unknown, userId: number) => {
      ...
      if (asyncDb) {
        ...
        for (const t of todos) {
          ...
          lists.push(
            `- ${t.title} (${completedCount}/${items.length}) - \`${t.id}\``,
          );
        }
  ...
  todo_show: {
    description:
      "Display the full details of a specific todo list including all items, their priorities, completion status, and progress. Use this when the user asks about a specific list or when you need to see the current state before making updates.",
    schema: z.object({
  ...
  todo_complete: {
    description:
      "Mark a single task as done/completed in a todo list. Use this when the user says they finished a task, completed something, or wants to check off an item. Requires the todo ID and the item index (1-based, so first item is index 0).",
    schema: MarkCompleteSchema,
    handler: async (
      args: z.infer<typeof MarkCompleteSchema>,
      userId: number,
    ) => {
      const todo = await getTodoWithItems(args.todoId, userId);
      ...
      if (asyncDb) {
        await asyncDb.run(
          "UPDATE todo_items SET completed = 1 WHERE todo_id = $1 AND item_order = $2",
          [args.todoId, args.itemIndex],
        );
        await asyncDb.run("UPDATE todos SET updated_at = $1 WHERE id = $2", [
          Math.floor(Date.now() / 1000),
          args.todoId,
        ...
        const db = getDatabase();
        db.run(
          "UPDATE todo_items SET completed = 1 WHERE todo_id = ? AND item_order = ?",
          [args.todoId, args.itemIndex],
        );
        db.run("UPDATE todos SET updated_at = ? WHERE id = ?", [
          Math.floor(Date.now() / 1000),
  ...
  todo_delete: {
    description:
      "Permanently delete a todo list and all its items. Use this when the user wants to remove, clear, or get rid of an entire todo list. This action cannot be undone.",
    schema: z.object({
    ...
    handler: async (args: { todoId: string }, userId: number) => {
      ...
      const asyncDb = getAsyncDatabase();
      if (asyncDb) {
        await asyncDb.run("DELETE FROM todos WHERE id = $1 AND user_id = $2", [
          args.todoId,
          userId,
        ]);
      } else {
